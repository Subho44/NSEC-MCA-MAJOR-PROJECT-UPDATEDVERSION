const CourseForm = ({ formData, handleChange, handleSubmit, handelFile, btnText }) => {
  return (
    <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-md space-y-4">
      ...
      <textarea name="description" placeholder="Description" value={formData.description} onChange={handleChange} rows="4" className="w-full border p-3 rounded-lg" required></textarea>
      <input type="file" multiple accept="image/*" onChange={handelFile} className="w-full border p-3 rounded-lg" />
      <button className="w-full bg-blue-600 text-white py-3 rounded-lg">{btnText}</button>
    </form>
  );
};
